...
# one status line, best-effort right-aligning the RIGHT part.
#
...
#
# Right-alignment is best-effort. The status-line subprocess is not a TTY and the
# terminal width is not provided, so width is probed via `tput cols </dev/tty`, then
# $COLUMNS, then width_fallback. Wide glyphs (block/dingbat) can drift the edge ±1-2.
...
# Terminal width: TTY probe -> COLUMNS -> fallback.
